...
class TroubleShootingModel extends Model
{
    ...
    {
        return $this->select('fullname, nama_distribusi')->selectCount('pembuat')->join('users', 'users.id = opl_trouble_shooting.pembuat')->join('distribusi', 'distribusi.id = opl_trouble_shooting.id_distribusi')->orderBy('pembuat', 'DESC')->groupBy('pembuat')->findAll(3);
    }
}
