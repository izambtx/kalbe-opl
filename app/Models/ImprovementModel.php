...
class ImprovementModel extends Model
{
    ...
    {
        return $this->select('fullname, nama_distribusi')->selectCount('pembuat')->join('users', 'users.id = opl_improvement.pembuat')->join('distribusi', 'distribusi.id = opl_improvement.id_distribusi')->orderBy('pembuat', 'DESC')->groupBy('pembuat')->findAll(3);
    }
}
