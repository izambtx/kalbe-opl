...
class PengetahuanDasarModel extends Model
{
    ...
    public function getCountApproved($id = false)
    {
        ...
        $onlyDeptEng = $this->where('status', 'Created')->whereIn('id_distribusi', $dept)->countAllResults();
    ...
    public function getCountMax($id = false)
    {
        ...
        return $this->select('fullname, nama_distribusi')->selectCount('pembuat')->join('users', 'users.id = opl_pengetahuan_dasar.pembuat')->join('distribusi', 'distribusi.id = opl_pengetahuan_dasar.id_distribusi')->orderBy('pembuat', 'DESC')->groupBy('pembuat')->findAll(3);
    }
}
