            ...
            <div class="card text-center shadow mx-auto table-responsive mb-5">
                <table class="table table-striped table-bordered font-weight-bold">
                    ...
                    <tbody class="text-gray-900">
                        ...
                        <?php foreach ($troubleshootingUser as $tsR) : ?>

                            <tr>
                                <td class="align-middle" scope="row"><?= $i++; ?></td>
                                <td class="w-50 align-middle"><?= $tsR['tema'];  ?></td>
                                <td class="align-middle"><span class="font-weight-bold text-gray-900"><?= $tsR['nama_distribusi'];  ?></span></td>
                                <?php if ($tsR['mesin'] == '1') : ?>
                                    <td class="align-middle"><span class="font-weight-bold text-gray-900"><?= $tsR['nama_mesin'];  ?></span></td>
                                <?php else : ?>
                                    <td class="align-middle"><span class="badge badge-primary"><?= $tsR['nama_mesin'];  ?></span></td>
                                <?php endif; ?>

                                <td class="align-middle"><?= date('d M Y', strtotime($tsR['created_at'])); ?></td>
                                <td class="align-middle">
                                    <a class="btn btn-sm rounded-circle" href="/List-Sosialisasi-TroubleShooting/Detail-Sosialisasi-TroubleShooting/<?= $tsR['id']; ?>" role="button">
